# DailyMint - Technical Stack & Implementation Guide
...
- **Farcaster Frame SDK** (Core miniapp functionality)
- **OnchainKit** (Wallet connection & transaction components)
- **Tailwind CSS** + **shadcn/ui** (Neobrutalism components)
...
- **Neynar API** (Farcaster data & social features)
...
```css
/* app/globals.css - Replace existing content */
@tailwind base;
@tailwind components;
@tailwind utilities;
...
```typescript
// Use the installed neobrutalism components directly
...
### 6. Claude AI Integration for Prompts
...
export class ClaudeService {
  ...
  async generateDailyPrompt(type: 'text' | 'visual' | 'idea' | 'micro_fiction'): Promise<string> {
    ...
    try {
      ...
      // Fallback prompts if Claude API fails
  ...
  async enhanceUserCreation(content: string): Promise<{
    ...
    try {
      const message = await this.anthropic.messages.create({
        model: "claude-3-haiku-20240307",
        max_tokens: 200,
        system: "You are a supportive creative writing assistant. Analyze the user's writing and provide encouraging feedback.",
        messages: [{
          role: "user",
          content: `Please analyze this creative writing and respond with JSON format:
          {
  ...
  async generateCollectionName(creations: string[]): Promise<string> {
    try {
      ...
      const message = await this.anthropic.messages.create({
        ...
        messages: [{
          role: "user",
          content: `Based on these sample writings, suggest a collection name:\n\n${sampleContent}`
        }]
      });
      
      return message.content[0].type === 'text' ? message.content[0].text.trim() : 'My Creative Journey';
    } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingPrompt.data) {
      return NextResponse.json(existingPrompt.data);
    }
    
    // Generate new prompt with Claude
...
```typescript
// app/page.tsx - Main Miniapp Entry
'use client';
...
4. Add domain to Farcaster manifest
